...
@endsection
@section('title')
<a href="{!! route('admins.product.list', ['projectId' => $obj->product->project_id ]) !!}">Product</a>
<a href="{!! route('admins.apartment.list', ['product' => $obj->product->id ]) !!}">Apartment</a> > Status Transaction
@endsection
@section('content')
<!-- tile -->
<section class="tile">
    ...
    <div class="tile-body">
        <div class="panel-body handling">
            <div class="search filter-search">
                <form class="navbar-form navbar-left form-search" action="{{ route('admins.apartment.searchTransaction') }}" method="GET">
                    <input type="hidden" name="_token" value="{{csrf_token()}}" />
                    <input type="hidden" id="apartmentId" name="apartmentId" value="{{ $obj->id }}" />
                    ...
                    <a href="{!! route('admins.apartment.status',['id' => $obj->id]) !!}" role="button" tabindex="0" style="margin-left: 15px; text-decoration: none" class="form-control" class="pull-right">Show all</a>
                </form>
            </div>
        </div>
